...
describe method(:inotify_watch) do
  ...
  it "is a convenience wrapper for NotifyHub.watch" do
    `mkdir -p #{$testdir}`
    
    prc = Proc.new {nil}
    w1 = NotifyHub.watch($testdir, :modify, :dont_follow, &prc)
    w2 =   inotify_watch($testdir, :modify, :dont_follow, &prc)
    w1.class             .must_equal w2.class
    w1.instance_variables.must_equal w2.instance_variables
    (w1.instance_variables-[:@callback]).each do |sym|
      a = w1.instance_variable_get(sym)
